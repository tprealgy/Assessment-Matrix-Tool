document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('addBulkAssignmentBtn').addEventListener('click', async () => {
    const name = document.getElementById('assignmentInput').value.trim();
    if (!name) return alert('Ange ett uppgiftsnamn!');

    const selectedRequirements = [...document.querySelectorAll('.area-checkbox-container input[type="checkbox"]:checked')]
    ...
    if (confirm(`Skapa uppgiften "${name}" för alla studenter i kursen?`)) {
      const response = await fetch(`/api/assignments/bulk?course=${courseName}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        alert(`Uppgiften har skapats för ${result.studentsUpdated} studenter!`);
        document.getElementById('assignmentInput').value = '';
        document.querySelectorAll('.area-checkbox-container input[type="checkbox"]').forEach(cb => cb.checked = false);
        ...
        alert('Kunde inte skapa uppgiften.');
      }
    }
  });
  ...
  document.getElementById('undoLastAssignmentBtn').addEventListener('click', async () => {
    if (!currentStudentId) return;

    // First get the assignment name to show in confirmation
    const previewRes = await fetch(`/api/students/${currentStudentId}/assignments/last?course=${courseName}`);
    if (previewRes.ok) {
      const preview = await previewRes.json();
      if (preview.success && preview.assignmentName) {
        if (confirm(`Är du säker på att du vill ångra uppgiften "${preview.assignmentName}"?`)) {
          const res = await fetch(`/api/students/${currentStudentId}/assignments/undo-last?course=${courseName}`, {
            method: 'DELETE'
          });
          ...
          if (res.ok) {
            ...
            if (result.success) {
              alert(`Uppgiften "${result.assignmentName}" har ångrats.`);
              loadStudentData(currentStudentId);
            } else {
              alert('Inga uppgifter att ångra.');
            }
          } else {
            alert('Kunde inte ångra uppgiften.');
          }
        }
      } else {
        alert('Inga uppgifter att ångra.');
      ...
      alert('Kunde inte hämta uppgiftsinformation.');
    }
  });
  ...
  function populateAssignmentForm() {
    ...
    let checkboxContainer = document.querySelector('.area-checkbox-container');
    if (!checkboxContainer) {
      checkboxContainer = document.createElement('div');
      checkboxContainer.className = 'area-checkbox-container';
      areaSelect.parentElement.appendChild(checkboxContainer);
    }
    
    checkboxContainer.innerHTML = '';
    
    assessmentAreas.forEach((area, index) => {
      // Keep the original select for compatibility
      const option = document.createElement('option');
      ...
      checkboxItem.className = 'area-checkbox-item';
  ...
  async function init() {
    ...
    initHelpSystem('index', courseName);
  }
