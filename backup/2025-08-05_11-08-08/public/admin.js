document.addEventListener('DOMContentLoaded', () => {
  ...
  const standardColors = [
    ...
    '#34495e', '#e67e22', '#95a5a6', '#16a085', '#8e44ad', '#2980b9',
    '#27ae60', '#f1c40f', '#c0392b', '#7f8c8d'
  ];
  ...
  function createColorSwatches() {
    ...
    standardColors.forEach(color => {
      ...
      swatch.style.border = '3px solid #e9ecef';
      ...
      swatch.addEventListener('click', () => {
        document.querySelectorAll('.color-swatch').forEach(s => {
          ...
          s.style.border = '3px solid #e9ecef';
          s.style.transform = 'scale(1)';
        });
        swatch.classList.add('active');
        swatch.style.border = '3px solid #3498db';
  ...
  async function loadStudents() {
    ...
    students.forEach(student => {
      ...
      li.innerHTML = `
        ...
        <div>
          <button class="action-btn toggleHideBtn" data-id="${student.id}" data-hidden="${isHidden}" title="${isHidden ? 'Visa student' : 'Dölj student'}">${isHidden ? '👁️' : '🙈'}</button>
          <button class="action-btn deleteStudentBtn" data-id="${student.id}" title="Ta bort student">🗑️</button>
        </div>
      `;
    ...
    areas.forEach((area, index) => {
      ...
      li.innerHTML = `
        ...
        <div class="area-controls">
          <button class="action-btn moveUpBtn" data-index="${index}" title="Flytta upp">⬆️</button>
          <button class="action-btn moveDownBtn" data-index="${index}" title="Flytta ner">⬇️</button>
          <button class="action-btn deleteBtn" data-index="${index}" title="Ta bort">🗑️</button>
        </div>
      `;
    ...
    if (currentCourse) {
      ...
      document.querySelectorAll('.color-swatch').forEach(s => {
        ...
        if (s.dataset.color === selectedColor) {
          ...
          s.style.border = '3px solid #3498db';
  ...
  function init() {
    ...
    initHelpSystem('admin', courseName);
  }
