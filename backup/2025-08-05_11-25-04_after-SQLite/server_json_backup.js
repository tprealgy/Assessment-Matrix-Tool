...
app.get('/api/app-settings', (req, res) => {
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    res.json({}); // Return empty object if no settings file exists
  }
});
...
app.delete('/api/app-settings', (req, res) => {
  const settingsPath = path.join(__dirname, 'data', 'app_settings.json');
  try {
    if (fs.existsSync(settingsPath)) {
...
app.delete('/api/courses/:courseName', (req, res) => {
    ...
    if (fs.existsSync(coursePath)) {
        try {
            ...
            fs.writeFileSync(path.join(newPath, '.deletion_info.json'), JSON.stringify(deletionInfo, null, 2));
...
function cleanupOldDeletedCourses() {
    const deletedCoursesPath = path.join(__dirname, 'data', 'deleted_courses');
    if (!fs.existsSync(deletedCoursesPath)) return;

    const sixMonthsAgo = new Date();
    sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6);

    try {
        const dirents = fs.readdirSync(deletedCoursesPath, { withFileTypes: true });
        dirents.filter(dirent => dirent.isDirectory()).forEach(dirent => {
            const coursePath = path.join(deletedCoursesPath, dirent.name);
            const deletionInfoPath = path.join(coursePath, '.deletion_info.json');
            
            if (fs.existsSync(deletionInfoPath)) {
                const deletionInfo = JSON.parse(fs.readFileSync(deletionInfoPath));
                const deletedAt = new Date(deletionInfo.deletedAt);
                
                if (deletedAt < sixMonthsAgo) {
                    console.log(`Cleaning up old deleted course: ${dirent.name}`);
...
app.get('/api/deleted-courses', (req, res) => {
    ...
    // Run cleanup before returning courses
    cleanupOldDeletedCourses();
        ...
        const courses = dirents
            ...
            .map(dirent => {
                ...
                if (fs.existsSync(deletionInfoPath)) {
                    try {
                        const deletionInfo = JSON.parse(fs.readFileSync(deletionInfoPath));
                        const deletionDate = new Date(deletionInfo.deletedAt);
                        deletedAt = deletionDate.toLocaleDateString('sv-SE');
                        
                        // Calculate expiration date (6 months from deletion)
                        ...
                        console.error(`Error reading deletion info for ${dirent.name}:`, e);
                    }
                }
...
app.post('/api/courses/restore', (req, res) => {
    ...
    const coursePath = path.join(__dirname, 'data', 'courses', targetCourseName);
    ...
    if (fs.existsSync(deletedCoursePath)) {
        try {
            ...
            // If renamed, update the course metadata
            if (newCourseName && newCourseName !== courseName) {
                const metaPath = path.join(coursePath, 'course_meta.json');
                if (fs.existsSync(metaPath)) {
                    const meta = JSON.parse(fs.readFileSync(metaPath));
                    meta.displayName = newCourseName;
                    fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2));
                } else {
                    // Create new metadata if it doesn't exist
                    saveCourseMeta(targetCourseName, { displayName: newCourseName, color: '' });
                }
            }
...
app.post('/api/assignments/bulk', (req, res) => {
  ...
  if (!requirements || !Array.isArray(requirements) || !assignmentName) {
    return res.status(400).json({ error: 'Invalid data for bulk assignment.' });
  }
  ...
  // Add assignment to all students (visible ones only)
  students.filter(student => !student.hidden).forEach(student => {
    requirements.forEach(requirement => {
      ...
      if (reqIndex !== -1) {
        ...
        if (!student.assignments[reqIndex].E) {
          student.assignments[reqIndex].E = [];
        }
        // Add to E level with grey color (no color)
        student.assignments[reqIndex].E.unshift({ 
          name: assignmentName, 
          color: 'grey', 
          createdAt: newAssignmentTimestamp 
        });
      }
    });
  });
  ...
  saveStudents(courseName, students);
  res.status(200).json({ success: true, studentsUpdated: students.filter(s => !s.hidden).length });
});
