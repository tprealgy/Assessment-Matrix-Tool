...
class Database {
  constructor() {
    // Use app root folder for portable data storage
    let userDataPath;
    if (__dirname.includes('app.asar')) {
      // Running as packaged Electron app - use executable directory/data
      const execPath = process.execPath;
      const appDir = path.dirname(execPath);
      userDataPath = path.join(appDir, 'data');
    } else {
      // Running in development - use local data folder
      userDataPath = path.join(__dirname, 'data');
    }
    this.dbPath = path.join(userDataPath, 'matris.db');
    this.db = null;
  }
